    ...
    <div class="card p-4 shadow rounded-4 bg-light bg-opacity-75">
        <form action="{{ route('app.animais.update', $animal->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="brinco_chip" class="form-control" value="{{ $animal->detalhes->brinco_chip }}">
            </div>

            <div class="mb-3">
                <label for="peso_nascimento" class="form-label">Peso ao nascer (kg)</label>
                <input type="number" name="peso_nascimento" class="form-control" value="{{ $animal->detalhes->peso_nascimento }}" step="0.01">
            </div>

            <div class="mb-3">
                <label for="peso_atual" class="form-label">Peso atual (kg)</label>
                <input type="number" name="peso_atual" class="form-control" value="{{ $animal->detalhes->peso_atual }}" step="0.01">
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="rebanho_id" class="form-select">
                    ...
                    @foreach($rebanhos as $rebanho)
                        <option value="{{ $rebanho->id }}" {{ $animal->detalhes->rebanho_id == $rebanho->id ? 'selected' : '' }}>
                            {{ $rebanho->nome }}
                        </option>
                    @endforeach
                </select>
